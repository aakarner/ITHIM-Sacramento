...
nTrafficModeV <- 8 #victim mode
nTrafficModeS <- 9 #striking mode (include NOV)
nInjuriedType <- 2 #fatal & serious
...
createDistribution <- function(scenario.no = 1){
  ...
  precent.person.baseline[,1]<-c(0.75,0.53,0.364,0.17756911,NA,0.17756911,NA,0.17756911)
  precent.person.baseline[,2]<-c(0.25,0.47,0.626,0.25345185,NA,0.25345185,NA,0.25345185)
  precent.person.baseline[,3]<-c(0.0000000667,0.0000000667,0.01,0.56897904,NA,0.56897904,NA,0.56897904)
...
#obtain the ITHIM sheet "Dist by rode type" after inputting the scenario number (1,2,3 or 4, using 4 as an example here)
dist <-  createDistribution(4) 
